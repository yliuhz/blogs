...
作者在3个DP任务上验证假设：最大度、最短路和$n$-body问题。首先，考虑计算图的最大度，可以通过1步DP解决。作为定理1的一个推论，使用求和聚合函数的GNNs无法在该问题上良好泛化。

**推论1**：使用求和聚合和求和读出的GNNs在最大度问题上不能泛化。
...
作者通过实验验证定理3和推论1。作者发现在最大度任务上，使用$\max$读出函数的GNN确实比使用$\sum$读出函数的GNN的泛化效果要好；在最短路任务上，使用$\min$读出函数的GNN也确实比使用$\sum$读出函数的GNN要好（图(a)）。
